...
namespace Ultimate.AI
{
	...
	public class UltimateAI : MonoBehaviour
	{
		...
		[Tooltip("Maximum distance at wich the AI is able to pic? a random point and walk to it.")]
			...
			{   //If the effect is set to slowing:
				if (slownessFactor != 0) //And the value is not equal to 0 (You cannot divide by 0!):
				{
					if (player.GetComponent<FPSController>()) //Remember to change "FPSController" to your controller's class name!!
					{
						...
						if (!infected)
						{
							player.GetComponent<FPSController>().walkSpeed /= slownessFactor;   //Remember to change "FPSController" to your controller's class name!!
							player.GetComponent<FPSController>().sprintSpeed /= slownessFactor;  //Also you should replace "walkingSpeed" & "runningSpeed" with what 																	
